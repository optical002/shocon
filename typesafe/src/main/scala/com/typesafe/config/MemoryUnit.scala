...
/** Memory-size units, mirroring `com.typesafe.config.impl.MemoryUnit`. Ported from the old facade. */
...
object MemoryUnit {
    ...
    values.foreach { unit =>
      ...
      if (unit.prefix.length() == 0) {
        ...
        if (unit.powerOf == 1024) {
          ...
          if (unit.power == 1) map += first + "B" -> unit // 512kB
          else map += firstUpper + "B" -> unit // 512MB
        }
      }
    }
    map.result()
  ...
  def parseUnit(unit: String): Option[MemoryUnit] = unitsMap.get(unit)
}
